...
public class AreaPanelManager : MonoBehaviour
{
    ...
    private readonly List<AreaPanel> panels = new List<AreaPanel>();
    ...
    public void SetAreas(int[] xs,int[] ys)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            for (int i = 0; i < 3; i++)
            {
                Vector3 min = GridGenerator.IndexToWorld(xs[i], ys[j]);
                Vector3 max = GridGenerator.IndexToWorld(xs[i + 1], ys[j + 1]);
